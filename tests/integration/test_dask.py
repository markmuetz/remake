"""Dask executor — LocalCluster workers, sidecar results, per-rule barriers.
...
def test_dask_end_to_end(pipeline_dir, capsys):
    assert cli('run', 'pipeline.py', '-E', 'dask', '-j', '2') == 0
...
def test_dask_failure_skips_downstream(pipeline_dir, capsys):
...
rmk.rules_from_current_module()
''')
    assert cli('run', 'failing.py', '-E', 'dask', '-j', '2') == 1
    assert Path('data/f_1.txt').exists()  # independent task still ran
    assert Path('data/g_1.txt').exists()  # untainted element ran
    assert not Path('data/g_2.txt').exists()  # tainted element skipped
...
def test_dask_needs_remakefile():
    from remake import DaskExecutor, Remake, RemakeError
