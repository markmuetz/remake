...
class Archive(BaseArchive):
    ...
    def archive(self, dry_run):
        ...
        print(f'Total file size: {humanbytes(total_size)}')
        ...
        if not dry_run:
            with tarfile.open(Path(f'{self.archive_loc}') / f'{self.name}.remake_archive.tar.gz', 'w:gz') as tar:
                for prepend, path, shorten in [(prepend, Path(path), shorten) for prepend, path, shorten in file_manifest]:
                    if shorten:
                        ...
                        tar.add(str(path), str(Path(prepend) / path))

def restore(archive_file):
