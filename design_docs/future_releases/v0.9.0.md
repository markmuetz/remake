...
section by value-for-effort. One item — **RO-Crate export** (Ecosystem) — is
remake-native rather than Snakemake-derived, folded in here because it shares
the run-report's "serialise the metadata DB" machinery and lands the same
provenance theme. See [roadmap.md](../roadmap.md) for how 0.9.0 sits in the
wider plan.
...
- **RO-Crate export (provenance/archival).** Package a completed pipeline's
  outputs + metadata + provenance as an
  [RO-Crate](https://www.researchobject.org/ro-crate/) for publication and
  archival — the successor to remake2's archive feature. The key realisation
  (full mapping in [discussion.md](../discussion.md)) is that remake **already
  holds** nearly everything RO-Crate wants — rules, tasks, kwargs, input/output
  paths, code/`uses`/`io` hashes, timestamps, status — so this is mostly a
  *serialiser* over the existing DB, sharing machinery with the run report
  (both render the metadata DB to a single artefact). This is a strong
  differentiator for remake's scientific-workflow niche: reproducible,
  citable provenance for large pipelines, near-free given the data already
  recorded. Promoted into 0.9.0 (2026-06-23).
...
benchmark capture** + **DAG export** + **run report** + **RO-Crate export**
form a tight "observability, correctness & provenance" theme that reuses the
existing DB and token machinery (the report and the crate are two renderers of
the same metadata), with **execution profiles** as the migration-facing extra.
