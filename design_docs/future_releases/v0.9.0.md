...
>    metadata DB and adds peak RSS, for all executors. **Design pass done
>    2026-07-31 — [resource_capture.md](../resource_capture.md).** Decided
>    there: last-execution `wall_s`/`cpu_s`/`max_rss_bytes`/`rss_method`
>    columns on `task` in `remake.db` (the append-only per-execution
>    `stats.db` of discussion.md stays 0.10.x), measured in `run_task` — the
>    one chokepoint all executors share — by a `/proc` sampler thread, since
>    `ru_maxrss` is a process high-water mark and would mis-attribute a
>    pooled worker's earlier peak. Output bytes are *not* duplicated here:
>    they are `output_stat` (item 6). Rides the shared early migration with
>    items 2 and 6.
> 2. **Output validation** (`ensure`-style on the token ABC) — second; may
>    want a small schema addition, which should land early. **Checksum
>    split (decided 2026-07-14, MM):** 0.9.0 *records* output checksums —
>    a `checksum` column riding the same early `output_stat` migration as
>    item 6, computed opt-in (`checksum_outputs=True` per rule/global,
>    `Ensure(checksum=True)` per output; never default — hashing re-reads
>    every byte) at the post-success hook in the task process (page-cache
>    hot; SLURM carries it via sidecar/ingest), sha256 for
>    provenance-interop, per-file in `Dir` manifests under the same knob.
>    Everything that *reads* a checksum is 0.10.x: `verify`-style
>    commands, RO-Crate (which needs the 0.9-era history — capture can't
>    wait), and any checksum-based staleness, which is a NEW RERUN
>    TRIGGER and gets its own design pass.
