...
### Per-task jobs (remake2 behaviour)

Each task gets its own `sbatch` script. The dependency chain is expressed with
`--dependency=afterok:JOBID`. This gives maximum pipeline throughput — a
downstream task starts as soon as its specific upstream task completes.

### Array jobs (new in remake3)

When a rule's tasks are independent of each other (no intra-rule dependencies)
*and* all tasks map 1-to-1 to a downstream rule's tasks (same matrix), remake3
submits an array job instead:

```bash
sbatch --array=0-40 extract.sbatch        # 41 years of ERA5
sbatch --array=0-40 --dependency=aftercorr:1234 anomalies.sbatch
```

`aftercorr` wires array element N of the downstream job to element N of the
upstream job — so `anomalies[era5,1981]` waits only for `extract[era5,1981]`,
not the whole array.

The executor decides which submission mode to use per rule:

```
use_array = True
  if rule has no intra-rule task dependencies
  and all depends_on rules use the same matrix
  and the matrix size exceeds a configurable threshold (default: 10)
```

When a fan-in occurs (e.g. `aggregate` over all years), the downstream rule
falls back to individual jobs with `--dependency=afterok:JOBID,...` listing all
array element job IDs.
...
Global defaults come from `Remake(config={'slurm': {...}})`.
